...
#R work directory
setwd("E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup") # May have to adapt that
...
mu.bLinf ~ dnorm(0, 0.01)
tau.bLinf~ dgamma (0.5,0.1)

mu.bLmat ~ dnorm(0, 0.01)
...
tau.Linf ~ dgamma(5,0.1)
tau.Lmat ~ dgamma(5,0.1)
tau.r~dgamma(0.01,0.01)

sigma_r<-sqrt(1/tau.r)
sigma_Tmax<-sqrt(1/tau.Tmax)
sigma_Linf<-sqrt(1/tau.Linf)
sigma_Lmat<-sqrt(1/tau.Lmat)
...
est.r[i] <- bTmax[group[i]]/estTmax[i]+pow(estLinf[i],bLinf[group[i]])+pow(estLmat[i],bLmat[group[i]])
...
# Bundle data
win.data <- list("r","Tmax","Linf","Lmat","group","n")

# Function to generate starting values
inits <- function()
  list (
    mu.bTmax=1,tau.bTmax=1,
    mu.bLinf=1,tau.bLinf=1,
    mu.bLmat=1,tau.bLmat=1,
    bTmax=c(1,1,1),
    bLinf=c(1,1,1),
    bLmat=c(1,1,1),
...
params <- c("bTmax","bLinf","bLmat",
            "mu.bTmax","tau.bTmax","tau.Tmax",
            "mu.bLinf","tau.bLinf","tau.Linf",
            "mu.bLmat","tau.bLmat","tau.Lmat",
            "tau.r","sigma_r","sigma_Tmax","sigma_Linf","sigma_Lmat",
            "bpvalue","predicted", 
            "estTmax","estLinf","estLmat","residual")
...
# require(devEMF)
# emf("E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\residual plot mix without beta0.emf",
#     width = 6.7, height = 3.35)

pdf('E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\figure\\figure 8.pdf',width=7, height=3.8,onefile = FALSE)
par(mfrow=c(1,2))
...
library(xlsx)
write.xlsx(stat,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "stat",row.names=T)
write.xlsx(coe,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "coe",row.names=T,append=T)
write.xlsx(result,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "result",row.names=T,append=T)
write.xlsx(loo_mat,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "loo_estimates",row.names=T,append=T)

# # require(devEMF)
# emf("E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.emf",
#     width = 3.35, height = 3.35)
# observed=r
# BHEIV_predicted=out$mean$predicted#BHEIV model predicted r
# plot(observed,col="red",pch=1, main = "rTmaxLinfLmatgroup mix without beta0",cex.main=0.8,
#      xlab = "index",ylab="r")
...
write.xlsx(r_com,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "r_com",row.names=T,append=T)
...
p_com=data.frame(Tmax,est_Tmax,
                 Linf,est_Linf,
                 Lmat,est_Lmat)
write.xlsx(p_com,"E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\20200312BHEIV\\rTmaxLinfLmatgroup\\mix without beta0.xlsx",sheetName = "p_com",row.names=T,append=T)
...
data=data.frame(out$sims.list$bLmat);names(data)=c("b1Lmat","b2Lmat","b3Lmat")
library(ggplot2)
cols=c('Invertebrate'="#f04546", 'Elasmobranch'="#33CC33", 'Teleost'="#3591d1")
shapes=c('Invertebrate'=1,'Elasmobranch'=3,'Teleost'=5)
density_plot3=
...
library(magrittr)
library(ggpubr)
pdf('E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\figure\\figure 9.pdf',width=8, height=3,onefile = FALSE)
# png( paste0('E:\\01-doctor_LCJ\\09-fishery\\02-r_LHPs paper\\figure\\figure 1.png'), width= 20, height=12, unit='cm', res=300)
ggarrange(density_plot1,density_plot2,density_plot3,labels = c("A", "B","C"), 
          font.label=list(size=10,face="plain"),ncol = 3, nrow =1,common.legend = TRUE, legend = "top")
